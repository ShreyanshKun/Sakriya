...
interface Rule { text: string; icon: React.ReactNode; type: 'info' | 'warning'; }
interface Accommodation { id: number; name: string; price: string; images: string[]; description: string; capacity: string; bedrooms: number; bathrooms: number; amenities: string[]; features: string[]; rules?: Rule[]; }

interface BookingModalProps { isOpen: boolean; onClose: () => void; accommodation: Accommodation | null; }
...
export default function BookingModal({ isOpen, onClose, accommodation }: BookingModalProps) {
  ...
  const handleSendBooking = () => {
    ...
    const YOUR_PHONE_NUMBER = "918144218850"; // <-- IMPORTANT: Change this!

    // 2. Create the pre-filled message using state
...
Can you please confirm availability?`;
    ...
    // This tells the browser to open the WhatsApp link.
                ...
                {accommodation.images.map((img, index) => (
                  <CarouselItem key={index} className="h-full">
                    <div className="relative h-full w-full">
                      <Image src={img} alt={`${accommodation.name} image ${index + 1}`} fill className="object-cover"/>
                    </div>
                  </CarouselItem>
                ))}
              </CarouselContent>
                  ...
                  <div className="bg-amber-50 border-l-4 border-amber-400 p-4 rounded-r-lg space-y-3">
                    {accommodation.rules.map((rule, index) => (
                      <div key={index} className={`flex items-start space-x-3 ${rule.type === 'warning' ? 'text-red-700' : 'text-gray-700'}`}>
                        <div className="flex-shrink-0 mt-0.5">{React.cloneElement(rule.icon as React.ReactElement, { className: "h-5 w-5" })}</div>
